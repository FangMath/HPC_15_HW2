...
//float beta;
int beta; // the data type of message sent/received should match
MPI_Request reqs[10];
MPI_Status stats[10];
...
if (rank == 1) {
  for (i=0; i<10; i++) {
    ...
    MPI_Irecv(&beta, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &reqs[i]); // the data type of message sent/received should match
    MPI_Wait(&reqs[i], &stats[i]);
    //printf("Task %d received = %f\n",rank,beta);
    printf("Task %d received = %d\n",rank,beta); // the data type of message sent/received should match
    }
  }
